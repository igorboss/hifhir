...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        if (serverAddr == nil || serverAddr == "") {defaults.set( "https://fhir.nortal.com/blaze", forKey: "FHIRserver");}
        
        let patientID = defaults.string(forKey: "PatientID")
        if (patientID == nil || patientID == "") {defaults.set( "Patient/1", forKey: "PatientID");}
        defaults.synchronize()
        
        let firstView: FirstViewController = FirstViewController()
        firstView.healthStore = self.healthStore
